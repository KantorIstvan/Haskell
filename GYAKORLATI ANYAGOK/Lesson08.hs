...
data Weather = Sunny | Cloudy | Rainy Int | Snowy Int deriving (Show, Eq)
...
-- b) Akkor is jó idő volt kirándulni, ha a csapadék mennyisége kevesebb volt, mint 2 mm!

niceDayForHiking (Rainy x) = x < 2
niceDayForHiking (Snowy x) = x < 2
...
daytimeAvg temps = sum daytimeTemps / fromIntegral (length daytimeTemps)
  where
    daytimeTemps = [temp | Daytime temp <- temps]
